...
try {
    ...
    if ($method === 'GET') {
        ...
        // Se não tem parâmetro 'data', verificar se há parâmetros diretos no formato D{id}N{nivel}
        if (empty($data)) {
            // Verificar todos os parâmetros GET para encontrar um no formato D{id}N{nivel}
            foreach ($_GET as $key => $value) {
                if (preg_match('/^D(\d+)N(\d+(?:\.\d+)?)$/', $key, $matches)) {
                    $data = $key;
                    break;
                }
            }
        ...
        if (preg_match('/^D(\d+)N(\d+(?:\.\d+)?)$/', $data, $matches)) {
            ...
            if ($action === 'list') {
                ...
                returnJson([
                    'success' => false,
                    'message' => 'Formato inválido. Use um dos formatos: ?data=D{id}N{nivel}, ?D{id}N{nivel} ou ?action=list',
                    'examples' => [
                        'Enviar medição (formato 1): ?data=D1N75.5',
                        'Enviar medição (formato 2): ?D1N75.5',
